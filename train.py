...
def main():
    ...
    if args.use_lr_scheduler:
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[500, 1000, 1500, 1750, 2000, 2500], gamma=0.9)
    ...
    for epoch in range(args.epochs):
        ...
        for _, image_batch in enumerate(loop):
            ...
            # 计算损失
            edge_loss_value = mse_loss(image_gradient(outputs, gray=args.gray), image_gradient(lable, gray=args.gray))
            pixel_loss_value = mse_loss(outputs, lable)
            ssim_loss_value = 1 - ssim_loss(outputs, lable, normalize=True)

            total_loss = pixel_loss_value + args.ssim_weight[1] * ssim_loss_value + edge_loss_value
            ...
            if step % args.tensorboard_step == 1:
                with torch.no_grad():
                    ...
                    writer.add_scalar('edge_loss', edge_loss_value.item(), global_step=step)
